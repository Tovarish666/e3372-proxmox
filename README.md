...
**Универсально по режиму переключения.** Часть E3372h-153 переключается в HiLink сама
(сразу `12d1:14dc`), часть приходит как CD-ROM (`12d1:1f01`) и требует usb_modeswitch.
Инсталлер кладёт generic Huawei-конфиг с `HuaweiNewMode=1` — он срабатывает **только** на
модемах в CD-ROM (авто-переключающихся не касается, нулевой риск), а конкретный HiLink-PID
модем выбирает сам (14dc у h-153, 1575 у K5160 и т.д.). Один конфиг покрывает весь парк.
Модемы в **stick**-режиме (напр. непрошитый s-153, `ttyUSB`) HiLink'ом не станут — нужна
HiLink-прошивка (делается на ноуте, вне scope).
...
| 2 | Ставит `usb-modeswitch(+data)` и `networkd-dispatcher` **офлайн** из локального репо (apt ставит только недостающее, установленное новее — не трогает); кладёт generic Huawei ZeroCD→HiLink конфиг |
